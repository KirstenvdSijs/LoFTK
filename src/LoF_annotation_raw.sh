...
# if ANALYSIS_TYPE empty 
	...
	do
		...
		if [ ! -d ${ROOTDIR}/${PROJECTNAME}_Files_for_VCF_LoF/vcf_chr"$chr" ]; then
		    ...
		    echoerrorflash "This directory was empty from the beginning: ${ROOTDIR}/${PROJECTNAME}_Files_for_VCF_LoF/vcf_chr"$chr". "
		fi
...
#	    echo "mv ${ROOTDIR}/${PROJECTNAME}_Files_for_VCF_LoF/vcf_chr*/*vep.vcf.gz ${OUTPUTDIR}" >> ${OUTPUTDIR}/run_vep_to_lof_gene.sh
		...
		echo "${PERL} /LoFTK/src/vep_vcf_to_gene_lofs.pl -v -o ${PROJECTNAME}_gene.lof" >> ${OUTPUTDIR}/run_vep_to_lof_gene.sh
	    elif [ ${FILE_FORMAT} == "VCF" ]; then
		cp ${LOFTK}/src/vep_vcf_to_gene_lofs_vcf.pl ${OUTPUTDIR}
		echo "sleep 15" >> ${OUTPUTDIR}/run_vep_to_lof_gene.sh
		echo "${PERL} /LoFTK/src/vep_vcf_to_gene_lofs_vcf.pl -v -o ${PROJECTNAME}_gene.lof" >> ${OUTPUTDIR}/run_vep_to_lof_gene.sh
	    else
                echoerrorflash "                        *** ERROR *** "
                echoerrorflash "Something went wrong. You must have to set the FILE_FORMAT in LoF_config to either IMPUTE2 or VCF. "
                echoerrorflash "                *** END OF ERROR MESSAGE *** "
...
#=====================#
		...
		echo "${PERL} /LoFTK/src/vep_vcf_to_snp_lofs.pl -v -o ${PROJECTNAME}_snp.lof" >> ${OUTPUTDIR}/run_vep_to_lof_snp.sh
	    elif [ ${FILE_FORMAT} == "VCF" ]; then
                cp ${LOFTK}/src/vep_vcf_to_snp_lofs_vcf.pl ${OUTPUTDIR}
		echo "${PERL} /LoFTK/src/vep_vcf_to_snp_lofs_vcf.pl -v -o ${PROJECTNAME}_snp.lof" >> ${OUTPUTDIR}/run_vep_to_lof_snp.sh
            else
                echoerrorflash "                        *** ERROR *** "
                echoerrorflash "Something went wrong. You must have to set the FILE_FORMAT in LoF_config to either IMPUTE2 or VCF. "
                echoerrorflash "                *** END OF ERROR MESSAGE *** "
